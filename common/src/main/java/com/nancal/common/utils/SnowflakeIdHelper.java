...
public class SnowflakeIdHelper {
    private static SnowflakeIdHelper snowflakeId = new SnowflakeIdHelper(1);
    ...
    private SnowflakeIdHelper(long id) {
    ...
    public static SnowflakeIdHelper getFlowIdInstance() {
        return snowflakeId;
    }
...
//            log.error(String.format("clock moved backwards.Refusing to generate id for %d milliseconds", (this.lastTimestamp - timestamp)));
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 100; i++) {
            SnowflakeIdHelper helper = SnowflakeIdHelper.getFlowIdInstance();
            System.out.println(helper.nextId());
        }
    }
